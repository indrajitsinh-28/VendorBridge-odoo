...
import {
  ...
  Download, AlertCircle, Search, Receipt,
} from "lucide-react";
import { useTheme } from "../context/ThemeContext";

interface InvoiceItem { name: string; qty: number; unit: string; price: number; }
interface AddressBox { orgName: string; address: string; gstin: string; }
type InvoiceStatus = "Draft" | "Pending Payment" | "Paid" | "Overdue";
...
const INVOICES: Invoice[] = [
  ...
  },
  {
    ...
    dueDate: "17 Mar, 2025",
    billTo: { orgName: "VendorBridge Procurement Corp", address: "Plot 7, Sector 44, Gurugram, Haryana – 122003", gstin: "06AABCV1234A1Z5" },
    vendor: { orgName: "NexaIT Solutions", address: "Embassy Tech Village, Outer Ring Road, Bengaluru – 560103", gstin: "29AABCN7890E1Z9" },
    items: [
      { name: "Annual SaaS licence — ERP module", qty: 1, unit: "Licence", price: 320000 },
      { name: "Implementation support (40 hrs)", qty: 40, unit: "Hours", price: 3500 },
    ],
    status: "Draft",
    paymentMethod: "Bank Transfer (NEFT)",
  },
...
const STATUS_STYLES: Record<InvoiceStatus, string> = {
  ...
  "Pending Payment": "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20",
...
export function Invoices() {
  ...
  const filtered = useMemo(() =>
    invoices.filter(inv =>
      ...
      inv.poRef.toLowerCase().includes(searchQuery.toLowerCase()) ||
      inv.vendor.orgName.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const markAsPaid = () => {
    setInvoices(prev => prev.map(inv =>
      inv.id === selected.id ? { ...inv, status: "Paid" as InvoiceStatus } : inv
    ));
    toast(`Invoice ${selected.id} marked as Paid.`);
  };

  const openEmail = () => {
    setEmailTo(selected.vendor.orgName.toLowerCase().replace(/\s+/g, "") + "@vendor.com");
    setEmailSubject(`Invoice ${selected.id} — Payment Request from VendorBridge`);
    setEmailBody(`Dear Finance Team,\n\nPlease find attached Invoice ${selected.id} (ref: ${selected.poRef}) dated ${selected.invoiceDate}.\n\nAmount Due: ₹${totals.grandTotal.toLocaleString("en-IN", { minimumFractionDigits: 2 })}\nPayment Due: ${selected.dueDate}\nPayment Method: ${selected.paymentMethod}\n\nKindly process payment at the earliest.\n\nBest regards,\nArjun Kapoor\nProcurement Manager, VendorBridge`);
  ...
  const handleSendEmail = (e: FormEvent) => {
    ...
    toast(`Invoice ${selected.id} sent to ${emailTo} successfully.`);
  };
  ...
  const navItems = [
    ...
    { name: "Purchase Orders", icon: DollarSign, path: "/purchase-orders" },
    { name: "Invoices", icon: FileDown, path: "/invoices", active: true },
        ...
        {navItems.map((item) => {
          ...
          return (
            <button key={item.path} onClick={() => { navigate(item.path); setIsMobileSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${"active" in item && item.active ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"}`}>
        ...
        <header className="sticky top-0 z-40 bg-card border-b border-muted/10 h-16 flex items-center justify-between px-6 print:hidden">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-muted uppercase tracking-wider hidden sm:block">Invoice Center</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
          ...
          <aside className="w-full md:w-80 border-r border-muted/10 bg-card overflow-y-auto flex-shrink-0 flex flex-col print:hidden">
            <div className="p-4 border-b border-muted/10 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-muted uppercase tracking-wider">Invoices</span>
                <div className="flex gap-1.5">
                  {(["Pending Payment", "Overdue"] as InvoiceStatus[]).map(s => {
                    const count = invoices.filter(i => i.status === s).length;
                    return count > 0 ? (
                      <span key={s} className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${STATUS_STYLES[s]}`}>
                        {count} {s === "Overdue" ? "overdue" : "pending"}
                      </span>
                    ) : null;
                  })}
                </div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search invoice, PO ref, vendor…" value={searchQuery}
            ...
            <nav className="flex-1 p-2 space-y-1">
              {filtered.map(inv => (
                <button key={inv.id} onClick={() => setSelectedId(inv.id)}
                  className={`w-full text-left p-3.5 rounded-xl transition-all duration-200 border ${inv.id === selectedId ? "bg-primary/5 border-primary/20" : "border-transparent hover:bg-primary/3"}`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-mono font-bold text-primary tracking-wide">{inv.id}</span>
                    <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-md border ${STATUS_STYLES[inv.status]}`}>{inv.status}</span>
                  </div>
                  <p className="text-xs font-bold text-foreground truncate">{inv.vendor.orgName}</p>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-[10px] text-muted font-medium">Ref: {inv.poRef}</span>
                    <span className="text-[10px] font-mono font-bold">
                      ₹{inv.items.reduce((a, i) => a + i.qty * i.price, 0).toLocaleString("en-IN")}
                    </span>
                  </div>
                  <p className="text-[10px] text-muted mt-1.5">Due {inv.dueDate}</p>
                </button>
              ))}
            </nav>
          </aside>
          ...
          <main className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6 print:p-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 border-b border-muted/10 pb-5 print:hidden">
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1 rounded-full border ${STATUS_STYLES[selected.status]}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[selected.status]}`} />
                    {selected.status}
                  </span>
                </div>
                <p className="text-xs text-muted mt-1 font-mono">{selected.id} · Ref {selected.poRef} · Due {selected.dueDate}</p>
              </div>
            ...
            <div className="bg-card rounded-2xl border border-muted/10 p-6 sm:p-8 shadow-sm space-y-8 print:border-0 print:shadow-none">
              ...
              <div className="hidden print:block border-b border-gray-300 pb-4">
                <h1 className="text-xl font-bold uppercase text-black">TAX INVOICE</h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-background rounded-2xl border border-muted/5">
                {[
                  { label: "Invoice No.", value: selected.id },
                  { label: "Invoice Date", value: selected.invoiceDate },
                  ...
                  { label: "Line Items", value: `${selected.items.length} items` },
                  { label: "Vendor", value: selected.vendor.orgName },
                ].map(({ label, value }) => (
                  <div key={label}>
                    <span className="text-[10px] text-muted font-semibold uppercase tracking-wide block">{label}</span>
                    <span className="text-xs font-bold text-foreground mt-0.5 block truncate">{value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-background rounded-2xl border border-muted/5">
                ...
                <div className="space-y-1.5 text-xs md:border-l md:border-muted/10 md:pl-6">
                  <span className="text-[10px] text-muted font-bold uppercase tracking-wide block">Billed By (Vendor):</span>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-muted/10 bg-card text-muted uppercase text-[10px] font-bold">
                      ...
                      <th className="px-5 py-3.5 text-right w-32">Rate</th>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-4 border-t border-muted/10 print:hidden">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-muted">via {selected.paymentMethod}</span>
                </div>
                {(selected.status === "Pending Payment" || selected.status === "Overdue") && (
                  <button onClick={markAsPaid}
                    className="h-10 px-5 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 transition-all active:scale-95 shadow-sm">
                    Mark as Paid
                  </button>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-card rounded-2xl border border-muted/15 shadow-2xl z-10 animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-muted/10 flex items-center justify-between">
              <h3 className="text-sm font-bold">Send Invoice</h3>
            ...
            <form onSubmit={handleSendEmail} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2 p-2.5 bg-primary/3 border border-muted/10 rounded-xl text-xs">
                ...
                <span className="text-foreground font-medium">{selected.id}.pdf (164 KB)</span>
              </div>
              <div className="flex justify-end gap-3 pt-3 border-t border-muted/10">
                ...
                <button type="submit"
                  className="px-5 h-10 bg-primary text-white text-xs font-bold rounded-xl hover:bg-primary/90 transition-all shadow-sm">Send Invoice</button>
              </div>
            </form>
          </div>
        </div>
